...
const config: XmcpConfig = {
  ...
  bundler: (config: any) => {
    ...
    // Ensure xmcp virtual modules are bundled, not externalized
    // These are generated by xmcp build and must be resolved to .xmcp/ directory
    // Note: xmcp config sandbox only allows "rspack" require, so use string concat
    const cwd = process.cwd();
    config.resolve.alias = {
      ...config.resolve.alias,
      "xmcp/headers": cwd + "/.xmcp/headers.js",
